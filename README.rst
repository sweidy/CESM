...
The replay is a version of the Incremental Analysis Update (IAU) used for performing data assimilation by several modeling centers, first described in Bloom et al., 1996. In standard data assimilation cases, the IAU is used to push the model towards observations; in the replay, the model is pushed towards a reanalysis. This can be useful for determining short-term model biases and evaluating model performance (see Schubert et al., 2019), among other use cases. 
...
The replay is currently implemented and used at NASA’s Global Modeling and Assimilation Office in the GEOS model. Here we implement the same tool in CESM2 using open source code, a simple startup, and adjustable replay parameters. Having the replay in multiple models allows for comparison of results across models, and the ability to easily assess short-term model drift in CESM. 
